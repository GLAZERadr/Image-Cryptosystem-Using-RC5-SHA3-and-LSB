...
class RC5Encryption(object):
    ...
    def encrypt_image(self, input_image_path, output_text_path):
        with open(input_image_path, "rb") as input_file:
            with open(output_text_path, "wb") as output_file:
                # Encrypt file and store in a binary buffer
                from io import BytesIO
                buffer = BytesIO()
                self.encrypt_file(input_file, buffer)
                buffer.seek(0)
                # Encode the buffer to base64 and write to text file
                base64_encoded = base64.b64encode(buffer.read())
                output_file.write(base64_encoded)

    def decrypt_image(self, input_text_path, output_image_path):
        with open(input_text_path, "rb") as input_file:
            with open(output_image_path, "wb") as output_file:
                # Read base64 encoded data and decode it
                base64_encoded = input_file.read()
                binary_data = base64.b64decode(base64_encoded)
                # Decrypt the binary data
                from io import BytesIO
                buffer = BytesIO(binary_data)
                self.decrypt_file(buffer, output_file)

# # Enkripsi Image to Cipherimages
# class RC5Encryption(object):
#     def __init__(self, key):
#         self.key = key
#         self.bs = 32
